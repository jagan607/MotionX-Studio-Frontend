    ...
    return (
        <div className="fixed inset-0 z-[65] flex items-center justify-center" onClick={close} style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            <div className="relative z-10 w-full max-w-lg mx-4 bg-[#1a1a1a] border border-white/[0.06] rounded-2xl overflow-hidden shadow-2xl"
                ...
                <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: "linear-gradient(90deg, transparent, #D40A12, transparent)" }} />
                <div className="p-7">
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-[#D40A12]/10 flex items-center justify-center"><Megaphone size={16} className="text-[#D40A12]" /></div>
                            <div><span className="text-[9px] font-mono text-[#D40A12] uppercase tracking-[4px] font-bold block">Welcome Back</span><span className="text-[10px] text-white/20">Here&apos;s what you missed</span></div>
                        </div>
                    ...
                    <div className="flex items-center justify-between mt-6 pt-5 border-t border-white/[0.03]">
                        {total > 1 ? (<div className="flex items-center gap-3"><button onClick={() => setIdx((idx - 1 + total) % total)} className="w-7 h-7 rounded-full border border-white/[0.06] flex items-center justify-center text-white/20 hover:text-white cursor-pointer bg-transparent"><ChevronLeft size={14} /></button><div className="flex gap-1.5">{announcements.map((_: any, i: number) => <button key={i} onClick={() => setIdx(i)} className={`rounded-full transition-all cursor-pointer border-none ${i === idx ? 'w-5 h-1.5 bg-[#D40A12]' : 'w-1.5 h-1.5 bg-white/10'}`} />)}</div><button onClick={() => setIdx((idx + 1) % total)} className="w-7 h-7 rounded-full border border-white/[0.06] flex items-center justify-center text-white/20 hover:text-white cursor-pointer bg-transparent"><ChevronRight size={14} /></button></div>) : <div />}
                        <button onClick={close} className="px-7 py-2.5 text-[10px] font-bold uppercase tracking-[3px] rounded-xl cursor-pointer border-none text-white" style={{ background: "linear-gradient(135deg, #D40A12, #B30710)", boxShadow: "0 4px 16px rgba(212,10,18,0.3)" }}>Got It</button>
                    </div>
                </div>
            </div>
        </div>
