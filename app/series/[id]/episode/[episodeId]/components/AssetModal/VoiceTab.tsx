...
import { Search, Loader2, Play, Square, Check, Mic } from 'lucide-react';
...
export const VoiceTab: React.FC<VoiceTabProps> = ({
    voiceSearch, setVoiceSearch, isLoadingVoices, filteredVoices,
    ...
    return (
        <div style={{ animation: 'fadeIn 0.3s ease', height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <button style={{ ...styles.primaryBtn, width: '100%', opacity: linkBtnState.disabled ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }} disabled={linkBtnState.disabled} onClick={handleVoiceSelection}>
                {isLinkingVoice ? <Loader2 className="animate-spin" size={16} /> : <Mic size={16} />} {linkBtnState.text}
            </button>
        </div>
    );
};