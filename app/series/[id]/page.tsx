...
export default function SeriesDetail() {
  ...
  const styles = {
    ...
    metaRow: { display: 'flex', gap: '30px', fontSize: '11px', color: '#D40A12', fontFamily: 'monospace', textTransform: 'uppercase' as const },
    addButton: { backgroundColor: '#EDEDED', color: '#111111', border: 'none', padding: '15px 30px', fontSize: '12px', fontWeight: 'bold' as const, letterSpacing: '2px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', textTransform: 'uppercase' as const, boxShadow: '0 0 20px rgba(255,255,255,0.1)' },
    ...
    episodeCard: { backgroundColor: '#161616', border: '1px solid #1A1A1A', padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', transition: 'all 0.2s', position: 'relative' as const },
    ...
    deck: { width: '900px', height: '600px', backgroundColor: '#111111', border: '1px solid #333', display: 'flex', boxShadow: '0 0 100px rgba(0,0,0,0.8)', position: 'relative' as const, overflow: 'hidden' },
    deckSidebar: { width: '260px', borderRight: '1px solid #222', padding: '30px', display: 'flex', flexDirection: 'column' as const, justifyContent: 'space-between', backgroundColor: '#161616', zIndex: 20 },
    ...
    menuItem: (active: boolean) => ({ display: 'flex', alignItems: 'center', gap: '12px', padding: '15px', marginBottom: '8px', backgroundColor: active ? '#111' : 'transparent', border: active ? '1px solid #333' : '1px solid transparent', color: active ? '#FFF' : '#666', fontSize: '11px', fontWeight: 'bold' as const, letterSpacing: '1px', cursor: 'pointer', transition: 'all 0.2s', borderLeft: active ? '2px solid #D40A12' : '2px solid transparent' }),
    ...
    executeBtn: { marginTop: '20px', width: '100%', padding: '20px', backgroundColor: '#D40A12', color: 'white', border: 'none', fontWeight: 'bold' as const, letterSpacing: '2px', fontSize: '12px', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px' }
  };
  ...
  return (
    <main style={styles.container}>
      ...
      <style>{`
        .ep-card:hover { border-color: #D40A12 !important; background-color: #0E0E0E !important; transform: translateY(-2px); }
        ...
        .animate-scanline { background: linear-gradient(to bottom, rgba(255,255,255,0), rgba(255,255,255,0) 50%, rgba(212,10,18,0.1) 50%, rgba(212,10,18,0.1)); background-size: 100% 4px; pointer-events: none; position: absolute; inset: 0; z-index: 10; opacity: 0.15; }
        .glow-text { text-shadow: 0 0 10px rgba(212,10,18,0.5); }
      `}</style>
          ...
          <div style={styles.deck}>
            ...
            <div style={styles.deckSidebar}>
              <div>
                ...
                <div style={styles.menuItem(inputMethod === 'synopsis')} onClick={() => setInputMethod('synopsis')} className="menu-hover"><Sparkles size={14} color={inputMethod === 'synopsis' ? '#D40A12' : '#666'} /> AI GENERATION</div>
              </div>
            ...
            <div style={styles.deckContent}>
              ...
              {inputMethod === 'upload' && (
                <div style={styles.uploadBox} className="upload-hover" onClick={() => fileInputRef.current?.click()}>
                  ...
                  {selectedFile ? (
                    <>
                      <Disc size={40} color="#D40A12" className="force-spin" style={{ animationDuration: '3s' }} />
              ...
              {inputMethod === 'synopsis' && (
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ fontSize: '10px', color: '#D40A12', marginTop: '10px', display: 'flex', gap: '6px', alignItems: 'center' }}><Cpu size={12} /> AI AGENT ACTIVE: STORY_ENGINE_V2</div>
                </div>
              )}

              <div style={{ marginTop: 'auto' }}>
                {isUploading && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', fontFamily: 'monospace', color: '#D40A12', marginBottom: '10px' }}>
