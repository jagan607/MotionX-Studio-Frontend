...
export const InpaintEditor = ({ src, onSave, onClose, onApply }: InpaintEditorProps) => {
    ...
    const draw = (e: React.MouseEvent) => {
        ...
        ctx.fillStyle = "rgba(212, 10, 18, 0.5)";
        ctx.beginPath();
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: '#111111', display: 'flex', flexDirection: 'column', fontFamily: 'monospace', color: '#EDEDED' }}>
            ...
            <div style={{ flex: 1, display: 'flex', padding: '20px', gap: '20px', overflow: 'hidden' }}>
                ...
                <div style={{ ...panelStyle, borderColor: outputImage ? '#D40A12' : '#222' }}>
                    <div style={{ ...badgeStyle, color: outputImage ? '#D40A12' : '#666' }}>{isProcessing ? 'RENDERING...' : outputImage ? 'FINAL RENDER' : 'AWAITING INPUT'}</div>
                    {isProcessing ? (
                        <div style={{ textAlign: 'center' }}>
                            <Loader2 className="animate-spin" size={40} color="#D40A12" />
            ...
            < div style={{ height: '80px', borderTop: '1px solid #333', backgroundColor: '#0A0A0A', display: 'flex', alignItems: 'center', padding: '0 20px', gap: '20px' }}>
                <div style={{ width: '150px' }}>
                    ...
                    <input type="range" min="5" max="100" value={brushSize} onChange={(e) => setBrushSize(Number(e.target.value))} style={{ width: '100%', accentColor: '#D40A12' }} />
                </div>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    <button onClick={handleGenerateFix} disabled={isProcessing} style={{ height: '40px', padding: '0 25px', backgroundColor: '#D40A12', color: '#FFF', border: 'none', fontSize: '11px', fontWeight: 'bold', letterSpacing: '1px', cursor: isProcessing ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '8px', opacity: isProcessing ? 0.5 : 1 }}>
