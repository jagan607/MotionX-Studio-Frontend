...
export const ShotImage = ({
    ...
    const [viewMode, setViewMode] = useState<'image' | 'video' | 'lipsync'>('image');
    const imgRef = useRef<HTMLImageElement>(null);

    const isAnimating = videoStatus === 'animating' || videoStatus === 'processing' || videoStatus === 'queued';
    const hasVideo = Boolean(videoUrl);
    const hasLipsync = Boolean(lipsyncUrl);
    ...
    // Auto-switch to video or lipsync when it becomes available
    useEffect(() => {
        if (hasLipsync) {
            setViewMode('lipsync');
        } else if (hasVideo) {
    ...
    // Resolve which media to show
    let activeMediaUrl = src;
    let isVideoMode = false;

    if (viewMode === 'lipsync' && lipsyncUrl) {
        activeMediaUrl = lipsyncUrl;
        isVideoMode = true;
    } else if (viewMode === 'video' && videoUrl) {
        activeMediaUrl = videoUrl;
        isVideoMode = true;
    }
        ...
        <div style={{ position: 'relative', width: '100%', height: '100%', backgroundColor: '#000', overflow: 'hidden' }}>
            ...
            {(hasVideo || hasLipsync) && !isLoading && (
                <div style={{
                    ...
                    {/* NEW: Explicitly checking hasLipsync to render the 3rd button */}
                    {hasLipsync && (
                        <button
                            onClick={(e) => { e.stopPropagation(); setViewMode('lipsync'); }}
                            style={{
                                padding: '4px 10px', borderRadius: '15px', border: 'none',
                                backgroundColor: viewMode === 'lipsync' ? '#00FF41' : 'transparent',
                                color: viewMode === 'lipsync' ? 'black' : '#888',
                                fontSize: '9px', fontWeight: 'bold', cursor: 'pointer',
                                display: 'flex', alignItems: 'center', gap: '4px',
                                transition: 'all 0.2s'
                            }}
