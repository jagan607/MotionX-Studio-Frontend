...
interface ScriptTimelineProps {
    ...
    onAddScene?: () => void;
    onDeleteScene?: (id: string) => Promise<void> | void; // Updated type to allow async
}

export const ScriptTimeline: React.FC<ScriptTimelineProps> = ({
    scenes, activeSceneId, episodeContext, onSetActiveScene, onReorder, onAddScene, onDeleteScene
}) => {
                    ...
                    {episodeContext && episodeContext.episodes.length > 0 ? (
                        <div className="flex items-center gap-3 w-full max-w-[70%]">
                            ...
                            <div className="flex-1 relative group bg-[#111] border border-[#222] hover:border-[#444] transition-colors rounded-sm h-8 flex items-center px-3">
                                ...
                                <select className="absolute inset-0 opacity-0 cursor-pointer w-full" value={episodeContext.currentEpisodeId} onChange={(e) => episodeContext.onSwitchEpisode(e.target.value)}>
                                    {episodeContext.episodes.map((ep) => <option key={ep.id} value={ep.id}>{ep.title}</option>)}
                                </select>
                            ...
                            <SortableContext items={scenes.map(s => s.id)} strategy={verticalListSortingStrategy}>
                                {scenes.map((scene, i) => (
                                    <SortableSceneCard
                                        ...
                                        // UPDATE: Trigger modal instead of direct delete
