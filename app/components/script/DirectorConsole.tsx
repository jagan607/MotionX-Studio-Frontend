...
export const DirectorConsole: React.FC<DirectorConsoleProps> = ({
    ...
    const locationBlock = (
        <div className="space-y-3">
            ...
            <div className={`grid ${isWide ? 'grid-cols-[2fr_1fr]' : 'grid-cols-[2fr_1fr]'} gap-3`}>
                {/* Location Select */}
    ...
    // Dialogues block
    const dialoguesBlock = activeScene.dialogues && activeScene.dialogues.length > 0 ? (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2 min-h-[40px] items-start">
                {activeCastList.map((charId: string) => (
                    <div key={charId} className="flex items-center gap-1.5 pl-3 pr-2 py-1.5 bg-[#1A1A1A] border border-[#333] text-white text-[10px] font-bold uppercase rounded-full group hover:border-[#555] transition-colors">
                        ...
                        <button onClick={() => handleRemoveCharacter(charId)} className="p-0.5 rounded-full hover:bg-white/20 text-[#666] hover:text-white transition-colors">
                            <X size={10} />
                        </button>
                    </div>
                ))}
    ...
    // Products block
    const productsBlock = availableProducts.length > 0 ? (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2 min-h-[40px] items-start">
                {activeProductList.map((prodId: string) => (
                    <div key={prodId} className="flex items-center gap-1.5 pl-3 pr-2 py-1.5 bg-[#1A1A1A] border border-[#333] text-white text-[10px] font-bold uppercase rounded-full group hover:border-[#555] transition-colors">
                        ...
                        <button onClick={() => handleRemoveProduct(prodId)} className="p-0.5 rounded-full hover:bg-white/20 text-[#666] hover:text-white transition-colors">
                            <X size={10} />
                        </button>
                    </div>
                ))}
    ...
    const moodBlock = (
        <div className="space-y-3">
            <button onClick={() => setMoodExpanded(!moodExpanded)} className="flex items-center justify-between w-full group">
                ...
                <div className="flex items-center gap-2">
                    {hasMoodData && !moodExpanded && (
                        <span className="text-[8px] font-mono text-amber-400/50 bg-amber-500/10 px-2 py-0.5 rounded uppercase tracking-wider">Set</span>
                    )}
                    <ChevronDown size={12} className={`text-white/30 transition-transform duration-200 ${moodExpanded ? 'rotate-180' : ''}`} />
                </div>
            </button>
            {moodExpanded && (
                <div className="space-y-3 bg-[#0A0A0A] border border-[#1E1E1E] rounded-lg p-3 shadow-[inset_0_1px_3px_rgba(0,0,0,0.2)]">
                    <div className="space-y-1">
                        ...
                        <input type="text" value={moodColorPalette} onChange={(e) => { setMoodColorPalette(e.target.value); setIsDirty(true); }} placeholder="e.g. Warm amber and deep gold..." className="w-full bg-[#0C0C0C] border border-[#1A1A1A] hover:border-[#333] text-[11px] text-white/80 px-3 py-2 rounded-md focus:outline-none focus:border-red-500/30 focus:shadow-[inset_0_1px_3px_rgba(0,0,0,0.3)] transition-all placeholder:text-[#333]" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-[9px] font-medium text-white/40 uppercase tracking-[0.12em] flex items-center gap-1.5">
                            <Sun size={10} className="text-white/25" /> Lighting
                        </label>
                        <input type="text" value={moodLighting} onChange={(e) => { setMoodLighting(e.target.value); setIsDirty(true); }} placeholder="e.g. Chiaroscuro, single source golden..." className="w-full bg-[#0C0C0C] border border-[#1A1A1A] hover:border-[#333] text-[11px] text-white/80 px-3 py-2 rounded-md focus:outline-none focus:border-red-500/30 focus:shadow-[inset_0_1px_3px_rgba(0,0,0,0.3)] transition-all placeholder:text-[#333]" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-[9px] font-medium text-white/40 uppercase tracking-[0.12em] flex items-center gap-1.5">
                            <Layers size={10} className="text-white/25" /> Texture
                        </label>
                        <input type="text" value={moodTexture} onChange={(e) => { setMoodTexture(e.target.value); setIsDirty(true); }} placeholder="e.g. Clean digital with lens flare..." className="w-full bg-[#0C0C0C] border border-[#1A1A1A] hover:border-[#333] text-[11px] text-white/80 px-3 py-2 rounded-md focus:outline-none focus:border-red-500/30 focus:shadow-[inset_0_1px_3px_rgba(0,0,0,0.3)] transition-all placeholder:text-[#333]" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-[9px] font-medium text-white/40 uppercase tracking-[0.12em] flex items-center gap-1.5">
                            <CloudFog size={10} className="text-white/25" /> Atmosphere
                        </label>
                        <input type="text" value={moodAtmosphere} onChange={(e) => { setMoodAtmosphere(e.target.value); setIsDirty(true); }} placeholder="e.g. Reverent, intimate, sacred..." className="w-full bg-[#0C0C0C] border border-[#1A1A1A] hover:border-[#333] text-[11px] text-white/80 px-3 py-2 rounded-md focus:outline-none focus:border-red-500/30 focus:shadow-[inset_0_1px_3px_rgba(0,0,0,0.3)] transition-all placeholder:text-[#333]" />
                    </div>
                </div>
            )}
        </div>
    ...
    // AI Assistant block
    const aiBlock = (
        <div className={`${isWide ? '' : 'pt-6 border-t border-[#111]'} space-y-4`}>
            <div className="flex items-center justify-between">
                ...
                <div className="px-6 py-4 bg-[#050505] border-t border-[#111] shrink-0 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            disabled={!isDirty}
                            className="px-5 py-2.5 text-[10px] font-bold tracking-widest uppercase text-white/60 hover:text-white bg-white/[0.06] hover:bg-white/[0.10] border border-white/[0.08] rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
