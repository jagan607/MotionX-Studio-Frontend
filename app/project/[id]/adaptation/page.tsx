    ...
    return (
        <>
            ...
            <div className="fixed inset-y-0 right-0 w-[420px] bg-[#0A0A0A] border-l border-[#333] z-[70] flex flex-col shadow-2xl">
                <div className="h-16 border-b border-[#222] flex items-center justify-between px-6 bg-[#1a1a1a] shrink-0">
                    <div className="flex items-center gap-3">
                        <ImagePlus className="text-[#D40A12]" size={18} />
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    {currentUrl ? (
                        <div className="space-y-3">
                            ...
                            <div className="relative rounded-lg overflow-hidden border border-[#333] group">
                                ...
                                <div className="absolute top-2 left-2 bg-[#D40A12]/80 text-white text-[7px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-wider backdrop-blur-sm">Active</div>
                            </div>
                        </div>
                    ) : null}
                    ...
                    <div className="space-y-3">
                        ...
                        <div
                            ...
                            className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-all group
                                ${isDragging ? 'border-[#D40A12] bg-[#D40A12]/[0.06] scale-[1.02]' : 'border-[#333] hover:border-[#D40A12]/50 hover:bg-[#D40A12]/[0.02]'}`}
                        >
                            <input
                                ref={fileRef}
                                type="file"
                            ...
                            {uploading ? (
                                <>
                                    <Loader2 size={28} className="text-[#D40A12] animate-spin mb-3" />
                                    <span className="text-[11px] text-neutral-400 font-bold">Uploading...</span>
                                </>
                            ) : (
                                <>
                                    <div className={`w-14 h-14 rounded-xl bg-white/[0.03] border flex items-center justify-center mb-3 transition-colors ${isDragging ? 'border-[#D40A12]/50' : 'border-white/[0.08] group-hover:border-[#D40A12]/30'}`}>
                                        <ImagePlus size={24} className={`transition-colors ${isDragging ? 'text-[#D40A12]' : 'text-neutral-600 group-hover:text-[#D40A12]'}`} />
                                    </div>
                    ...
                    <div className="space-y-2 border-t border-[#222] pt-5">
                        ...
                        <ul className="space-y-2 text-[11px] text-neutral-500">
                            <li className="flex items-start gap-2"><span className="text-[#D40A12] mt-0.5">•</span> Use a still from the look you want to achieve</li>
                            <li className="flex items-start gap-2"><span className="text-[#D40A12] mt-0.5">•</span> Film stills, paintings, or mood boards work well</li>
                            <li className="flex items-start gap-2"><span className="text-[#D40A12] mt-0.5">•</span> The AI extracts color palette, lighting direction, and atmosphere</li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    return (
        <>
            ...
            <div className="fixed inset-y-0 right-0 w-[500px] bg-[#0A0A0A] border-l border-[#333] z-[70] flex flex-col shadow-2xl slide-in-from-right duration-300">
                <div className="h-16 border-b border-[#222] flex items-center justify-between px-6 bg-[#1a1a1a]">
                    <h3 className="font-anton uppercase tracking-wide text-lg flex items-center gap-2 text-white">
                        <MousePointer2 size={18} className="text-[#D40A12]" /> Tagging Mode
                    </h3>
                    ...
                    <div className="relative border border-[#333] bg-black group cursor-crosshair">
                        ...
                        {tags.map((tag, i) => (
                            <div
                                ...
                                className={`absolute w-4 h-4 -ml-2 -mt-2 border-2 rounded-full cursor-pointer z-10 transition-all ${selectedTagIndex === i ? 'border-[#D40A12] bg-white scale-125' : 'border-white bg-black/50 hover:scale-110'}`}
                    ...
                    <div className="space-y-4">
                        ...
                        {selectedTagIndex !== null ? (
                            <div className="grid grid-cols-2 gap-2">
                                {clusters.map(c => (
                                    <button key={c.id} onClick={() => handleAssign(c.label_id)} className="flex items-center gap-3 p-2 bg-[#1a1a1a] border border-[#333] hover:border-[#D40A12] hover:bg-[#1a1a1a] rounded-sm transition-all text-left group">
                                        ...
                                        <div className="flex flex-col"><span className="text-[10px] font-bold text-white font-mono group-hover:text-[#D40A12]">CHAR #{c.label_id}</span></div>
                                    </button>
                                ))}
                            ...
                            <div className="space-y-2">{tags.map((tag, i) => (<div key={i} className="flex justify-between bg-[#1a1a1a] p-2 rounded border border-[#222]"><span className="text-xs">Marker {i + 1} → #{tag.label_id}</span><button onClick={() => handleRemove(i)}><X size={12} /></button></div>))}</div>
                        )}
                    </div>
                </div>
                <div className="p-6 border-t border-[#222] bg-[#1a1a1a] flex justify-end gap-3">
                    <button onClick={onClose} className="px-4 py-2 text-xs font-bold text-[#666]">CANCEL</button>
                    <button onClick={handleSave} className="bg-[#D40A12] text-black px-6 py-2 rounded-sm text-xs font-bold">SAVE & CLOSE</button>
                </div>
            </div>
        </>
    );
...
const SideBySideCard = ({ shot, index, onDelete, onRegenerate, onOpenTagging, onZoom, onInpaint }: any) => {
    ...
    return (
        <div className="bg-[#1a1a1a] border border-[#333] p-3 rounded-sm flex flex-col gap-3 group/card hover:border-[#444] transition-colors">
            {/* <GlobalHeader /> */}
            {/* Header */}
            ...
            <div className="flex gap-3 h-56">
                ...
                <div className="w-px bg-[#222] flex items-center justify-center"><div className="w-4 h-4 rounded-full bg-[#1a1a1a] border border-[#333] flex items-center justify-center text-[#444]"><ChevronRight size={10} /></div></div>

                {/* Right: Adaptation */}
                <div className="relative flex-1 bg-black border border-[#222] flex items-center justify-center overflow-hidden group">
                    {shot.status === 'rendering' ? (
                        <div className="flex flex-col items-center gap-3"><Loader2 className="animate-spin text-[#D40A12]" size={24} /><span className="text-[9px] text-[#666] animate-pulse font-mono tracking-widest">GENERATING</span></div>
                    ) : shot.video_url ? (
                        <>
                    ...
                    {shot.status === 'rendered' && <div className="absolute top-2 left-2 bg-[#D40A12] text-black text-[8px] font-bold px-1.5 py-0.5 font-mono rounded-sm shadow-lg">ADAPTATION</div>}
...
export default function AdaptationPage() {
    ...
    if (status.includes("analyzing") || status === "uploading") return <div className="h-screen bg-[#111111] flex justify-center items-center text-white"><Loader2 className="animate-spin text-red-600 mr-2" /> PROCESSING...</div>;
    if (status === "init") return <div className="h-screen bg-[#111111] flex justify-center items-center"><div className="border border-[#333] p-10"><input type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} className="text-white" /><button onClick={handleUpload} disabled={isUploading} className="bg-red-600 text-white p-2 mt-4 w-full">START ENGINE</button></div></div>;

    return (
        <div className="h-screen w-full bg-[#111111] text-white flex flex-col overflow-hidden relative">
            ...
            <div className="h-16 border-b border-[#222] flex items-center justify-between px-8 bg-[#0A0A0A] shrink-0">
                <div className="flex items-center gap-3">
                    {isProjectRendering ? <SplitSquareHorizontal className="text-[#D40A12] animate-pulse" size={20} /> : <User className="text-[#D40A12]" size={20} />}
                ...
                <div className="flex items-center gap-6">
                    {!status.includes('analyzing') && (
                        <div className="flex items-center gap-3 mr-4 border-r border-[#333] pr-6">
                            <button onClick={() => setShowStyleRef(true)} className="flex items-center gap-2 bg-[#1a1a1a] hover:bg-[#1a1a1a] border border-[#333] px-3 py-1.5 rounded-sm transition-all group cursor-pointer">
                                {styleRefUrl ? (
                                    <>
                                    ...
                                    <>
                                        <ImagePlus size={12} className="text-[#666] group-hover:text-[#D40A12]" />
                    ...
                    {isProjectRendering && <button onClick={handleReset} className="flex items-center gap-2 text-[10px] font-mono text-[#444] hover:text-[#D40A12] border border-[#222] hover:border-[#D40A12] px-3 py-1 rounded-sm transition-colors" title="Force Reset Status"><RefreshCw size={10} /> RESET STATUS</button>}
            ...
            {styleRefUrl && (
                <div className="shrink-0 border-b border-[#222] bg-[#0A0A0A] px-8 py-3 flex items-center gap-4">
                    <button onClick={() => setShowStyleRef(true)} className="flex items-center gap-4 group cursor-pointer">
                        <img src={styleRefUrl} className="w-20 h-12 rounded-md object-cover border border-[#333] group-hover:border-[#D40A12]/50 transition-colors shadow-lg" />
                        <div>
                            <div className="text-[9px] font-bold text-[#D40A12] uppercase tracking-[2px] mb-0.5">Style Reference</div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-12 dark-scrollbar">
                <div className={isProjectRendering ? "opacity-30 pointer-events-none filter grayscale transition-all duration-500" : "transition-all duration-500"}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {clusters.map((cluster) => (
                            <div key={cluster.id} className="bg-[#1a1a1a] border border-[#333] p-4 rounded-sm flex items-center gap-4 relative group hover:border-[#555] transition-colors">
                                ...
                                <div className="flex-1 h-16 border border-dashed border-[#333] bg-[#0A0A0A] rounded-sm flex flex-col items-center justify-center relative hover:border-[#D40A12] hover:bg-[#150505] transition-all cursor-pointer overflow-hidden">{uploadingCast === cluster.id ? <Loader2 className="animate-spin text-[#D40A12]" /> : cluster.new_face_url ? <img src={cluster.new_face_url} className="w-full h-full object-cover opacity-80" /> : <div className="text-[8px] font-mono uppercase text-[#666]">Assign Actor</div>}<input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" disabled={!!uploadingCast} onChange={(e) => { if (e.target.files?.[0]) handleCastUpload(cluster.id, e.target.files[0]); }} /></div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div>
                    ...
                    {isLoadingShots ? (
                        <div className="flex justify-center p-12"><Loader2 className="animate-spin text-[#D40A12]" /></div>
                    ) : (
